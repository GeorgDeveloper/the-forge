...
export class ProfessionComponent implements OnInit {
  ...
  protected readonly professionService = inject(ProfessionService);
  ...
  trackId = (item: IProfession): number => this.professionService.getProfessionIdentifier(item);
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.professions.set(dataFromBody);
  }

  protected fillComponentAttributesFromResponseBody(data: IProfession[] | null): IProfession[] {
    // If there is previous link, data is a infinite scroll pagination content.
    if (this.links().prev) {
      const professionsNew = this.professions();
      if (data) {
        for (const d of data) {
          if (professionsNew.some(op => op.id === d.id)) {
            professionsNew.push(d);
          }
        }
      }
      return professionsNew;
    }
    return data ?? [];
  }
